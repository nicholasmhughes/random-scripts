...
This is a hacked-up version of /var/lib/ambari-server/resources/scripts/configs.py that
will dump all of the Ambari configs to the current directory. It probably doesn't handle
errors very well...

python ./configs.py admin admin 8443 https get localhost HDF download_all
...
def main():
  ...
  if action == SET_ACTION:
    ...
    if config_type == 'download_all':
      ...
      for config_type in desired_tags["Clusters"]["desired_configs"].keys():
        #print config_type
        try:
          get_properties(cluster, config_type, [ config_type + '.conf' ], accessor)
        except:
          continue
    else:
